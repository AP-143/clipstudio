...
const SIZE = { S: 0.82, M: 1.0, L: 1.25 }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {renderTemplate(template, { badgeText, badgeColor, textColor, fontSize, scale, alignItems, text: config.text })}
      </div>
    </div>
  )
}

function badgePill(badgeText, badgeColor, fontSize, scale) {
  return (
    <span style={{
      backgroundColor: badgeColor, color: '#FFFFFF', fontFamily: FONT,
...
function badgeLabel(badgeText, badgeColor, fontSize) {
  return (
    <span style={{
...
function renderTemplate(template, p) {
  const { badgeText, badgeColor, textColor, fontSize, scale, alignItems, text } = p

  if (template === 'minimal') {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems, gap: `${9 * scale}px` }}>
        {badgeText && badgeLabel(badgeText, badgeColor, fontSize)}
        <span style={{
          fontFamily: FONT, fontSize, fontWeight: 800, color: textColor, lineHeight: 1.22,
          textShadow: '0 3px 14px rgba(0,0,0,0.65)', wordBreak: 'break-word',
        }}>{text}</span>
      </div>
    )
  }
  ...
  if (template === 'bar') {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems, gap: `${7 * scale}px` }}>
        {badgeText && badgeLabel(badgeText, '#FFFFFF', fontSize)}
        <span style={{
  ...
  if (template === 'outline') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems, gap: `${8 * scale}px` }}>
        {badgeText && badgePill(badgeText, badgeColor, fontSize, scale)}
        <span style={{
          fontFamily: FONT, fontSize: Math.round(fontSize * 1.08), fontWeight: 900,
          color: textColor, lineHeight: 1.15, textShadow: stroke, wordBreak: 'break-word',
        }}>{text}</span>
      </div>
    )
  }
  ...
  return (
    <div style={{
      ...
      alignItems: 'center', gap: `${13 * scale}px`,
    }}>
      {badgeText && badgePill(badgeText, badgeColor, fontSize, scale)}
      <span style={{
        fontFamily: FONT, fontSize, fontWeight: 800, color: '#0a0a0a',
        lineHeight: 1.25, wordBreak: 'break-word', textAlign: 'center',
      }}>{text}</span>
    </div>
  )
}
